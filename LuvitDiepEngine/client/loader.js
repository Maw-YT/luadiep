...
class ASMConsts {
    ...
    static createWebSocket(urlPtr) {
        ...
        const wsUrl = `ws${location.protocol.slice(4)}//${location.host}/${url.slice(5, url.length - 4)}`;
        console.log("[diepcustom] WASM url:", url, "→", wsUrl);
        ...
        ws.onmessage = function(e) {
            ...
            // Custom diepsound audio — consume in JS, do not forward to WASM
            if (view[0] === 0x0C && view.length >= 18 && window.DiepAudio) {
                const dv = new DataView(view.buffer, view.byteOffset, view.byteLength);
                const names = ["shot", "hit", "death", "shape"];
                const kind = names[view[1]] || "shot";
                const sx = dv.getFloat32(2, true);
                const sy = dv.getFloat32(6, true);
                const lx = dv.getFloat32(10, true);
                const ly = dv.getFloat32(14, true);
                window.DiepAudio.setListener(lx, ly);
                window.DiepAudio.play(kind, sx, sy);
                return;
            }
            if(view[0] === 7) {
                let out = 0, i = 0, at = 1;
